    ...
    <div id="comprobante" class="comprobante">
      ...
      <table>
        ...
        <tr v-for="(insumo, index) in insumos" :key="index">
          <td>{{ insumo.nombre }}</td>
          <td>{{ insumo.precio }}Bs x {{ insumo.cantidad }}</td>
          <td>{{ parseFloat(insumo.cantidad * insumo.precio) }}Bs</td>
        </tr>
      </table>
      <strong>TOTAL: {{ venta.total }}Bs</strong
      ><br />
      <strong>SU PAGO: {{ venta.pagado }}Bs</strong>
      <strong>CAMBIO: {{ venta.pagado - venta.total }}Bs</strong>
    </div>
  </section>
</template>
<script>
